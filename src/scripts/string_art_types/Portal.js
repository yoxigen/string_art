...
export default class Portal extends StringArt {
  ...
  getCalc() {
    ...
    const circleCenter = [radius + (size[0] - diameter) * circleX, radius + (size[1] - diameter) * circleY];

    return {
        nailDistance: Math.max(...size) / density,
        radius,
        circleCenter,
        ranges: this.getRanges({ size, radius, circleCenter })
    };
  }
  ...
  getRanges({ size, radius, circleCenter }) {
    ...
    const bottomRightDistanceFromCircleCenter = Math.sqrt((size[0] - circleCenter[0]) ** 2 + (size[1] - circleCenter[1]) ** 2);
    const angleToTopSide = Math.asin(radius / bottomRightDistanceFromCircleCenter) + Math.atan((size[0] - circleCenter[0]) / (size[1] - circleCenter[1]));
    ...
    // Right of top side
    const rightTopDistanceFromCircleCenter = Math.sqrt(circleCenter[1] ** 2 + (size[0] - circleCenter[0]) ** 2);
    const angleToLeftSide = Math.atan(circleCenter[1] / (size[0] - circleCenter[0])) + Math.asin(radius / rightTopDistanceFromCircleCenter);
    const bottomSideXFromRightTop = size[0] - size[1] / Math.tan(angleToLeftSide);
    ...
    // Top of left side
    const leftTopDistanceFromCircleCenter = Math.sqrt(circleCenter[0] ** 2 + circleCenter[1] ** 2);
    const angleToRightSide = Math.PI / 2 - (Math.asin(radius / leftTopDistanceFromCircleCenter) + Math.atan(circleCenter[0] / circleCenter[1]));
    ...
    // Left of bottom side
    const leftBottomDistanceFromCircleCenter = Math.sqrt(circleCenter[0] ** 2 + (size[1] - circleCenter[1]) ** 2);
    const angleFromTopSide = Math.atan((size[1] - circleCenter[1]) / circleCenter[0]) + Math.asin(radius / leftBottomDistanceFromCircleCenter);
...
console.log("RAGE", this.calc.ranges)
    ...
    //     const alpha = Math.atan(height / width) + Math.asin(radius * Math.sin(beta) / height);

    //     let destination = [this.size[0], point[1] - this.size[0] * Math.tan(alpha)];
    //     if (destination[1] < 0) {
    //         destination = [point[1] / Math.tan(alpha) + point[0], 0];
    //     } else if (destination[1] > this.size[1]) {
    //         destination = [(this.size[1] - point[1]) / Math.tan(Math.PI * 2 - alpha) + point[0], this.size[1]];
    //     }
